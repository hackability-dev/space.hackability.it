...
interface ContextUpload {
  handleUpload: (file: File, projectId: string) => Promise<void>;
}

const UploadContext = createContext<ContextUpload>({} as ContextUpload);
...
export const UploadProvider = ({ children }: { children: ReactElement }) => {
  ...
  const { mutateAsync: getGsUploadUrl } =
    trpc.project.generategsUploadUrl.useMutation();
  ...
  const setUpload = useCallback(
    (k: string, upload: Partial<UploadStatus>) => {
      setUploads((uploads) => {
        ...
        const newUp = { ...uploads[k], ...upload } as UploadStatus;
    ...
    try {
      await axios.request({
        ...
        onUploadProgress: (p: AxiosProgressEvent) => {
          const per = Math.round((p.loaded * 100) / (p.total || 100));
          setUpload(k, {
            progress: per,
          });
        },
      ...
      {!closed && (
        <div className="max-h-48 overflow-y-scroll">
          {entries.map(([key, status]) => (
            <div key={key} className="grid h-12 grid-cols-10 items-center p-2">
              <span className="col-span-2 overflow-hidden whitespace-nowrap text-xs ">
                {" "}
                {status.filename}
              </span>
